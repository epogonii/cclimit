...
function install() {
  ...
  if (existing?.type === 'command' && existing.command) {
    // Keep the user's statusline exactly as it was and put the collector in
    // front of it. It copies stdin through untouched, so the visible result is
    // identical to before.
    const script =
      '#!/bin/sh\n' +
      '# Generated by cclimit install. The line below is your original statusline\n' +
      '# command with the cclimit collector piped in front of it. Remove it with\n' +
      '# /cclimit uninstall, which also restores settings.json.\n' +
      `node ${JSON.stringify(SINK)} | ${existing.command}\n`;
    fs.writeFileSync(WRAP_FILE, script, { mode: 0o755 });
    settings.statusLine = { ...existing, command: WRAP_FILE };
  } else {
    settings.statusLine = { type: 'command', command: `node ${JSON.stringify(SINK)} --render`, padding: 0 };
  }
